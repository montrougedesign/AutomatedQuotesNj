...
class Stocks():
    ...
    def GetPrice(self,tick):
        ...
        result = f"{finvizfinance(tick).ticker_fundament()['Company']}\n"
    ...
    def GetAll(self,tick):
        ...
        file = open("AutomatedQuotes\AutomatedQuotesNj\AutomatedQuotes\AMQ Releases\Textfiles\data.txt", "r").read().split(",")
        headers = open("AutomatedQuotes\AutomatedQuotesNj\AutomatedQuotes\AMQ Releases\Textfiles\dataHeaders.txt", "r").read().split(",")
    ...
    def GetOne(self,tick,headType):
        ...
        for index, head in enumerate(headers) :
            ...
            if(head == headType.upper()):
                ...
                break
        return result
    def WatchLists(self,wl):    
        WLFILE = open("AutomatedQuotes\AutomatedQuotesNj\AutomatedQuotes\AMQ Releases\Textfiles\WL.txt", "r").read().split(",\n")
        wl = wl.upper()
        wl = int(wl.strip('WL'))
        
        if wl < len(WLFILE):
            ticks = WLFILE[wl].split(",")
            result =''
            for i in ticks:
    ...
    def News(self,tick):
        ...
        if len(rawNews) > 0:
            ...
            result +='No News'       
        return result      
    def Two(self,tick,from_):
        try:
            if tick[1] =="ALL":
                message = Stocks().GetAll(tick[0])
                Main().SendEmail(from_,message)
            elif tick[1] == "DHL":
                    message = Stocks().DayHighLow(tick[0])
                    Main().SendEmail(from_,message)
            elif tick[1] == "NEWS":
                    message = Stocks().News([0])    
                    Main().SendEmail(from_,message)
            else:
                message = Stocks().GetOne(tick[0],tick[1])
                Main().SendEmail(from_,message)
        ...
        if len(self.data) >0:
            for i, d in enumerate(self.data):
                ...
                if len(d) == 1:
                    ...
                    if  d[1] == "ADD":
                        message = d[2].replace("/",",")
                        f = open("AutomatedQuotes\AutomatedQuotesNj\AutomatedQuotes\AMQ Releases\Textfiles\WL.txt", "a")
                        f.write(f"{message},\n")
                        f.close()
                        self.SendEmail(self.froms[i],f" you added {message} to watchlist")   
                    else:
                        message = "Not an input"
                        self.SendEmail(self.froms[i],message)    
