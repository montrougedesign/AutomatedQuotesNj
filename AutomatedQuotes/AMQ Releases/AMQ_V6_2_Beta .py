...
class Stocks():
    ...
    def GetPrice(self, tick):
        ...
        fdata = requests.get(
            f"https://www.alphavantage.co/query?function=OVERVIEW&symbol={tick}&apikey=NBNC70MNF0ALLX8A").json()
    ...
    def GetOne(self, tick, headType):
        ...
        file = open("/root/Program/Data/data.txt", "r").read().split(",")
        headers = open("/root/Program/Data/dataHeaders.txt",
                       "r").read().split(",")
...
class DB():
    ...
    client = pymongo.MongoClient(
        "mongodb+srv://MYCoding:QV9BcLxtJqrInZB4@mycoding.pzucnk1.mongodb.net/?retryWrites=true&w=majority", tlsCAFile=ca)
    ...
    def getWLByUser(self, user, wlNum):
        db = self.client["Stocks"]
        collection = db["WL"]
        document = collection.find({'User': user}).skip(
            wlNum).limit(1)[0]["Tickers"].split(',')
        return document
...
class Main():
    def run(self):
        try:
            self.emailpro = Emails(
                'lakewoodnjQuotes@gmail.com', 'hlxnxrjvtgxeijky')
                    ...
                    if len(d) > 1 and d[-1] == "SEARCH":
                        ...
                        try:
                            ...
                            if d[1] == "ALL":
                                ...
                                self.SendEmail(
                                    self.froms[i], f"You added {message} to watchlist")
                            else:
    ...
    def GroupMe(self,chatName,message):
        c = Client.from_token(
                'P4q9frX8NOvojQcCyb0kjuLRAzOxX9H32NJHR73C')
    ...
    def SendEmail(self, from_, message):
        if from_ == '8482619494@mms.att.net':
            self.GroupMe('Chezy',message)
        elif from_ == '8482612817@mms.att.net':
            self.GroupMe('8482612817',message)
        else:            
